...
const Qwen05BInstruct = () => {
  ...
  const fineTuneOptions = ['LoRA', 'QLoRA', 'Full Parameter Fine-Tuning', 'Freeze'];
  ...
  const analysisData = {
    ...
    'Max Example': {
      ...
      summary: [
        ...
        "When fine-tuning Qwen2-0.5B-Instruct with Freeze, when the max sample is set to 250, the model does not reach a state of convergence, indicating that the amount of data is too small. With a max sample of 500, the model converges. When the max sample is increased to 1000, the training time lengthens, but there is little change in the loss on the validation set, and the validation set converges prematurely. Therefore, a max sample of 500 is chosen. "
      ]
    },
    'Learning Rate': {
      ...
      summary: [
        ...
        "When fine-tuning Qwen2-0.5B-Instruct with Freeze, when the learning rate is set to 5e-04 and 1e-04, overfitting occurs, as indicated by the increasing loss on the validation set. When the learning rate is set to 1e-05, the model has already converged, but the loss is higher compared to other learning rates, which might be due to the learning rate being too low and the model getting stuck in a local optimum. Therefore, the optimal learning rate is 5e-05."
      ]
    },
    'Epoch': {
      ...
      summary: [
        ...
        "When fine-tuning Qwen2-0.5B-Instruct with Freeze, when the epoch is set to 5, overfitting is evident from the loss curve of the validation set. However, when the epoch is set to 3, both the training set and the validation set have converged. Therefore, an epoch of 3 is chosen."
      ]
    }
  };
  ...
  const getLossChartSrc = (type) => {
    ...
    const src = `/pictures/Q0.5B_${method}_${paramSuffix}_${type}.png?${timestamp}`;
    return src;
  };
